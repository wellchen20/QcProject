...
public class MainActivity extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        externalId = findViewById(R.id.display_name);
        displayName = findViewById(R.id.register_external_id);
        ...
        if (islogin){
            ...
            externalId.setHint("用户唯一标识(如：UserId或者手机号)");
            displayName.setHint("呼叫中用户显示名称");
        }
        checkPermission();
    }
            ...
            nemoSDK.loginExternalAccount(displayName.getText().toString(), externalId.getText().toString(),
                    new ConnectNemoCallback() {
                        ...
                        public void onFailed(final int errorCode) {
                            ...
                            try {
                                runOnUiThread(() -> Toast.makeText(MainActivity.this, "匿名登录失败，错误码：" + errorCode, Toast.LENGTH_SHORT).show());
                            } catch (Exception e) {
                        ...
                        public void onNetworkTopologyDetectionFinished(LoginResponseData resp) {
                            ...
                            try {
                                runOnUiThread(() -> Toast.makeText(MainActivity.this, "探测完成", Toast.LENGTH_SHORT).show());
                            } catch (Exception e) {
                                L.e(TAG, e.getMessage());
                            }
                        }
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.connect_nemo) {
            ...
            nemoSDK.loginXYlinkAccount(displayName.getText().toString(), externalId.getText().toString(), new ConnectNemoCallback() {
                ...
                @Override
                public void onNetworkTopologyDetectionFinished(LoginResponseData resp) {
                    L.i(TAG, "net detect onNetworkTopologyDetectionFinished callback");
                }
            });
        }
    }

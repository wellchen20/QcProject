...
//识别模型
  ...
  public synchronized void draw(final Canvas canvas) {
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      // 屏幕实际宽度与预览宽度比值
      //---------------------手机屏幕分辨率适配--------------------------------
      double horizontalx = infact_width*1.0/frameWidth;
      double verticaly = infact_height*1.0/frameHeight;
      float y = (float) horizontalx;
      float x = (float) verticaly;
      //-----------------------------------------------------------------------
      RectF rect = new RectF(trackedPos.left*x, trackedPos.top*y,trackedPos.right*x, trackedPos.bottom*y);
      canvas.drawRect(rect, boxPaint);
      Log.i("baochaoqun", "old:" + trackedPos.left + "," + trackedPos.right + ", new:" + rect.left + "," + rect.right);
...
//      Log.e("labelString", "labelString: "+labelString );
      ...
      msg.obj=trackedPos.left+","+trackedPos.top+","+(trackedPos.right-trackedPos.left)+","+(trackedPos.bottom-trackedPos.top);
      msg.what=774;
...
//      Log.e("labelString", labelString );
      if(index==0)
      {
        ...
        new Thread(new Runnable() {
          @Override
          public void run() {
//            save2file(cdata,Environment.getExternalStorageDirectory().toString()+"/"+"img.yuv");

            seaveimg(cdata,frameWidth,frameHeight);

            Log.i("baichaoqun","labelString:" + labelString + ", trackedPos.left:"+trackedPos.left+", trackedPos.right"+trackedPos.right+"");
...
int infact_height;
  public synchronized void onFrame(
    ...
    if (objectTracker == null && !initialized) {
      ...
      frameWidth = w;
      frameHeight = h;
      infact_width = factWidth;
      infact_height = factHeight;
      this.sensorOrientation = sensorOrienation;
      initialized = true;

      logger.d("baichaoqun: onFrame-> frameWidth:" + w + ", frameHeight:" + h);
...
//    {// 获得图片
//      if (bytes!=null){
//        bMap = BitmapFactory.decodeByteArray(bytes, 0, bytes.length,null);
//        Matrix matrix = new Matrix();
//        matrix.setRotate(90);
//        bMap = Bitmap.createBitmap(bMap, 0, 0, width,
//                height, matrix, true);
//
...
//    {
//      e.printStackTrace();
//      Log.e("baichaoqun","存储失败"+e.toString());
//    }
//  }
  public void seaveimg(byte[] bytes,int width,int height)
  {

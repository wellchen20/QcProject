...
public class CarkeyBackActivity extends BaseActivity implements OnClickListener {

	private LinearLayout ll_backcarkey_carnum, ll_backcarkey_driver, ll_backcarkey_time;
	private TextView tv_backcarkey_carnum, tv_backcarkey_driver, tv_backcarkey_time;
	private Button btn_backcarkey_ok;
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case 0:
				if (!CarkeyBackActivity.this.isFinishing() && !progressDialog.isShowing()) {
					progressDialog.show();
				}
				break;
			...
			case 2:
				if (!isUpdate) {
					...
					CarkeyBackActivity.this.finish();
				}
				break;
...
//				}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_carkey_back);
		
		carKey = new CarKey();
		
		progressDialog = new ProgressDialog(CarkeyBackActivity.this);
	...
	private void initViews() {
		ll_backcarkey_carnum = (LinearLayout) findViewById(R.id.ll_backcarkey_carnum);
		ll_backcarkey_carnum.setOnClickListener(this);
		ll_backcarkey_driver = (LinearLayout) findViewById(R.id.ll_backcarkey_driver);
		ll_backcarkey_driver.setOnClickListener(this);
		ll_backcarkey_time = (LinearLayout) findViewById(R.id.ll_backcarkey_time);
		ll_backcarkey_time.setOnClickListener(this);
		tv_backcarkey_carnum = (TextView) findViewById(R.id.tv_backcarkey_carnum);
		tv_backcarkey_driver = (TextView) findViewById(R.id.tv_backcarkey_driver);
		tv_backcarkey_time = (TextView) findViewById(R.id.tv_backcarkey_time);
		btn_backcarkey_ok = (Button) findViewById(R.id.btn_backcarkey_ok);
		btn_backcarkey_ok.setOnClickListener(this);
		...
		((TextView) findViewById(R.id.tv_title_title)).setText(getResources().getString(R.string.back_carkey_title));
	}
	...
	private void initDatas() {
		...
		if (carnum != null && !"".equals(carnum)) {
			tv_backcarkey_carnum.setText(carnum);
		} else {
//			carnum = getData(SysContants.CARNUM, "");
			tv_backcarkey_carnum.setText("");
		}
		driver = getIntent().getStringExtra("driver");
		if (driver != null && !"".equals(driver)) {
			tv_backcarkey_driver.setText(driver);
		} else {
		...
		if (time != null && !"".equals(time)) {
			tv_backcarkey_time.setText(time);
		} else {
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case R.id.ll_backcarkey_carnum:
			Intent intent = new Intent(CarkeyBackActivity.this, MipcaActivityCapture.class);
			startActivityForResult(intent, 1);
			break;
			
		case R.id.ll_backcarkey_driver:
			
			break;
			
		case R.id.ll_backcarkey_time:
			UpdateTimeDialog();
			break;
	...
	private boolean sureOk() {
		...
		if (!"".equals(carnum)) {
			...
			Toast.makeText(CarkeyBackActivity.this, "请输入车牌号", Toast.LENGTH_SHORT).show();
			return isOk;
		}
		return isOk;
	}
...
//						proto_TravelImfor.toByteArray()));
	...
	public void UpdateTimeDialog() {
		new AlertDialog.Builder(CarkeyBackActivity.this)
		.setTitle("提示")
		.setMessage("是否更新时间")
